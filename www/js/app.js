...
.service('CourseService', function($q, $http, AppService, GeoService){
    ...
    service.GetWeatherSummary = function(courseID) {
      var deferred = $q.defer();
	    var promise = deferred.promise;
      $http({
        url: AppService.GetUrl('view-summary/id/{id}', {id: courseID}),
        method: "GET"
      }).success(function (data, status, headers, config) {
        console.log('GetWeatherSummary->success: ', data);
        deferred.resolve(data);
      }).error(function (data, status, headers, config) {
        console.log('GetWeatherSummary->error: ', data);
        console.log('GetWeatherSummary->error-status: ', status);
        console.log('GetWeatherSummary->error-headers: ', headers);
        deferred.reject(data);
      });
    ...
    service.ToggleFavourite = function(course) {
      var exists = false;
  		var index = -1;
      for(var i = 0; i < service.favourites.length; i++) {
        var _course = service.favourites[i];
